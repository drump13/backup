...
@sa exp_rp_tree
@detail
rg-tree->occurrence_list の子ノードのラベルを再帰的にまとめ上げていく,目盛り節約バージョン
*/
void exp_rg_tree_memred(RGTree* rg_tree,int min_sup,vector<Tree*> real_occs){
  ...
  for(int i = 0,n = real_occs.size();i<n;i++){
    for(int j = 0 , m = real_occs[i]->get_children().size(); j < m;j++){
      int l = real_occs[i]->get_children()[j]->get_node()->label;
      auto itr = label_map.find(l);
      if(itr == label_map.end()){
	vector<int> ilist;
	ilist.push_back(items[i]);
	label_map[l] = ilist;
	vector<Tree*> rolist;//real occurrence list
	rolist.push_back(real_occs[i]->get_children()[j]);
	label_occ_map[l] = rolist;
      }else{
	label_map[l].push_back(items[i]);
	label_occ_map[l].push_back(real_occs[i]->get_children()[j]);
      }
      
    }
  }
  //頻出なアイテムリストを持つノードに対して，親との差分を抽出
  map<int,vector<int>> real_label_map;
  for(auto itr = label_map.begin();itr!= label_map.end();++itr){
    if((itr->second).size() >= min_sup){
      vector<int> v;
      set_difference(items.begin(),items.end(),itr->second.begin(),itr->second.end(),back_inserter(v));
      real_label_map[itr->first]=v;
    }
  }

  //再帰的に処理
  for(auto itr = real_label_map.begin(); itr!=real_label_map.end(); ++itr){
    RGTree* rg_child = new RGTree(itr->first,itr->second);
    rg_child->set_parent(rg_tree);
    rg_tree->add_child(rg_child);
    exp_rg_tree_memred(rg_child,min_sup,label_occ_map[itr->first]);
...
*/
 /*
 EnumerationTree* get_closed_tree_memred(RGTree* rg_tree,CP* closed_pattern){
   EnumerationTree* ne = new EnumerationTree(rg_tree->get_node()->id,rg_tree->get_node()->label);
   //cout << "before make_closed" << (closed_pattern->occ_list).size()<< endl;
   make_closed_tree_rec_memred(rg_tree,closed_pattern->id_list,ne);
   //cout << "after make_closed " << endl;
   return ne;
   }*/
 ...
 */
 ...
 //他LCMの呼び出し，ファイル入出力は時間に入れない  
  ...
  vector<CP*> cp = convert_to_CP(r,rg_tree,is_memred);
  algorithm_restart();
  return cp;
}
...
vector<EnumerationTree*> SCC_Miner(TreeDB* db,EnumerationTree* constrainedTree, int minimum_support,bool rpcut_flag,bool is_trsact_based){
 ...
 /*
@brief SCC-Miner-memory_improvedアルゴリズム本体,RGTreeに親子の差分のみを残すことでメモリ削減している
@param1 db 木のデータベースへのポインタ
@param2 constrainedTree 部分木制約の木(ここは木リストに変更することも考えられる)
@param3 minimum_support ミニマムサポート
@param4 rpcut_flag rptreeの削減手法を適用するかどうか
@return 部分木制約を満たす頻出飽和な木パターンを全て含んだvector
@detail
1. db中のconstrainedTreeのポインタを全部列挙する
2. rp-treeを作る
...
6. 5で得られたパタンに対してrp-treeを見ながらフィルターをかける
 */
 vector<EnumerationTree*> SCC_Miner_Memory_Improved(TreeDB* db, EnumerationTree* constrainedTree, int minimum_support,bool rpcut_flag){
   vector<Tree*> oc_list = db->get_subtree_list(constrainedTree);
  vector<EnumerationTree*> result;
  if(oc_list.size() < minimum_support){return result;}
  RPTree* rp_tree = new RPTree(constrainedTree->get_node()->label,oc_list);
  ...
  for(int i = 0,n = root_candidates.size() ; i < n ; i++){
    RGTree* rg_tree = new RGTree(root_candidates[i]);
    exp_rg_tree_memred(rg_tree,minimum_support,rg_tree->get_occurrence_list());
    rg_tree->reindexing(0);
    //他作LCM
    vector<CP*> closed_patterns = callingLCM(convert(rg_tree->get_item_transaction_memred()),minimum_support,rg_tree,true);
