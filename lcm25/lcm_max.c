...
int LCMmax_first_iter( int item ){
  ...
  int i, ii, im, e, ee, n, addtail=LCM_add.t, js=LCM_jump.s, it, *perm;
  int flag=-1, flag2, cc=0, itemt, itemt2, itemt22, ff=0;
  QUEUE_INT *q, *qq;
...
/******************************************************/
  ...
  if ( ii<4 ){
    ...
    do {
      ...
      LCM_itemset2.t = itemt;
      LCMmax_iter ( &TT, &LCM_Occ, i, LCM_perm2, NULL, item, &jump2 );
      occ[i].t = occ[i].end = 0;
  ...
  LCMmax_iter ( &TT, &LCM_Occ, ee, LCM_perm2, NULL, item, &jump2 );
  occ[ee].t = 0;
  ...
  perm = LCMmax_alloc_occ ( &TT, &LCM_Occ, &OO, n, &LCM_jump, LCM_perm2, item );
  oocc = (QUEUE *)(OO.h);
  for ( i=0 ; i<OO.num ; i++ ) oocc[i].t = 0;
  LCMmax_replace_index_occurrence_deliver( &TT, &OO, LCM_ary[item] );
...
void LCMmax ( int n ){
  ...
  malloc2 ( LCM_perm2, int, LCM_Eend, "", "" );
