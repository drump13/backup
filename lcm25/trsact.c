...
ARY TRSACT_file_count ( char *fname, int *Tnum ){
  ...
  *Tnum=0; /* # transactions */
  ...
  b_end = b = buf_end = buf+TRSACT_BUFSIZ;
  ...
  while (1){
    do {
      ...
      while (1){
        if ( b == b_end ){
          if ( b_end < buf_end ) goto END;
          i = fread(buf, 1, TRSACT_BUFSIZ, fp);
          if ( i <= 0 ) goto END;
          b_end = buf + i;
          b = buf;
        }
  ...
  while (1){
    ...
    do {
      ...
      while (1){
        if ( b == b_end ){
          if ( b_end < buf_end ) goto END;
          ee = fread(buf, 1, TRSACT_BUFSIZ, fp);
          if ( ee <= 0 ) goto END;
          b_end = buf + ee;
          b = buf;
        }
...
void TRSACT_occ_deliver_all( ARY *T, ARY *O, QUEUE_INT max_item){
  int t;
  QUEUE_INT *x;
  QUEUE *Q =(QUEUE*)T->h, *QQ = (QUEUE*)O->h, *QQQ;
  for ( t=0 ; t<T->num ; t++ ){
...
void LCM_solution (){
  if ( LCM_print_flag&1 ) LCM_output_itemset_ ();
...
/*************************************************************************/
int LCM_init (char* input_file ,int min_sup ,char* output_file){
  int i, n=0, m, c=1;
  QUEUE *Q;
  /*
  if ( argc<3 ){
  ...
  LCM_perm = TRSACT_load ( input_file, &LCM_Trsact, LCM_th );
  ...
  LCM_sc[LCM_Eend] = 0;
  for ( Q=(QUEUE*)LCM_Trsact.h,i=0 ; i<LCM_Trsact.num ; i++ ){
  ...
  for ( i=0 ; i<LCM_Eend ; i++ ){
    LCM_occ[i].q = (QUEUE_INT *)malloc( sizeof(QUEUE_INT) * (LCM_occ[i].t+2) );
    if ( LCM_occ[i].q == NULL ){
...
int LCM_mk_freq_trsact( ARY *TT, ARY *T, ARY *O, QUEUE *occ, int cnt,
  ...
  int i, n=0;
  QUEUE *Q=(QUEUE*)T->h, *QQ, *OQ=(QUEUE*)O->h;
