...
//  int i, t, e, m, min_t = TRSACT_find_minsiz (&LCM_Trsact, occ);
  ...
  if ( flag&4 ){    // bitmap ppc (check items in increasing order)
    ...
    printf("%d ,%d\n",min_t,occ->t);
    for (m=Q[min_t].t-1; (e=Q[min_t].q[m])>item ; m-- ){
      printf("in for m = %d\n",m);
      LCM_Qand.q[m] = e;
    }
    LCM_Qand.q[m] = item;
...
void LCM_end (){
  ...
  printf("after fclose \n");
  QUEUE_end ( &LCM_jump );
  printf("after lcmjump end \n");
  QUEUE_end ( &LCM_itemset );
  printf("after lcm itemset end \n");
  QUEUE_end ( &LCM_add );
  printf("after lcm_add end \n");
  //for ( i=0 ; i<LCM_Eend ; i++ ) QUEUE_end( &LCM_occ[i] );
  printf("after queue ends \n");
  ARY_end ( &LCM_Occ );
  ARY_end ( &LCM_Trsact );
  printf("before frees \n");
...
void Mine_Closed_Itemsets(int minsup){
  int i, f=1;
  initialize();
  printf("before LCM_read_paramVA\n");
